...
exports.Adapter = {
    connected: function socketFileStoreConnected(d){
        ...
        self._db.serialize(function(){
            self._db.run("CREATE TABLE IF NOT EXISTS sockets (id INTEGER PRIMARY KEY, name TEXT NOT NULL UNIQUE, version INTEGER DEFAULT 1 NOT NULL, socket TEXT NOT NULL);", function (err) {
                if(err) done(err);

                self._db.run("CREATE TABLE IF NOT EXISTS socket_versions (socket_id INTEGER NOT NULL, name TEXT NOT NULL, version INTEGER NOT NULL, socket TEXT, FOREIGN KEY(socket_id) REFERENCES sockets(id), PRIMARY KEY (socket_id, version));", function(err){
                    if(err) done(err);
    ...
    replace: function socketFileStoreReplace(socket,cb){
        ...
        self._db.serialize(function(){
            self._db.run('INSERT into socket_versions (name, version, socket) select name, version, socket from sockets where id = ?;', [socket.id], function(err){
                if (err)
                    cb(err);

                self._db.run('UPDATE sockets set version = version + 1, name = ?, socket = ? where id = ?;', [s.name, JSON.stringify(s), s.id], function(err){
                    cb(err,this.changes);
                });
            });
        });
