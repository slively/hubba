...
app.controller('serviceCtrl',['$scope','hubba','$timeout','$location', function($, hubba, $timeout, $location){
    ...
    $.saveService = function saveNewServiceDef(s) {
        ...
        hubba.services[verb].apply(this,args).success(function(result){
            ...
            hubba.services.get();
            $location.path('/hubba-admin/services/'+ result.id);
        }).error(function(){
            $.serviceSaving = false;
        });
    };
...
app.controller('socketCtrl',['$scope','hubba','$timeout','$location', function($, hubba, $timeout, $location){
    ...
    $.saveNewSocketMethod = function saveNewSocketMethodDef(e) {
        if(e.keyCode === 13){
            hubba.selectedSocket.methods[$.newSocketMethodName.toString()] = "";
...
app.controller('resourceCtrl',['$scope', 'hubba', '$location', function($scope, hubba, $location) {
    ...
    $scope.typeChanged = function(){
        ...
        for (key in hubba.serviceTypes[hubba.selectedResource.type].configuration){
            hubba.selectedResource.configuration[key] = hubba.serviceTypes[hubba.selectedResource.type].configuration[key].value;
        }
    };
    ...
    if (typeof hubba.resources.cache.resourcesRoot == 'undefined'){
        ...
        hubba.serviceTypes.get(function(r){
            hubba.serviceTypes = r;
        });
