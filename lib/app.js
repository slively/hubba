...
function HubbaApp(opts) {
    ...
    HubbaStore.connect({type: 'file',name: appName, path: appRoot}, function(hStore){
        ...
        app.all('/api/login/:strategy?',function(req,res,next){
            if (req.method === "POST") {
                if (req.params.strategy) {
                    try {
                        passport.authenticate(req.params.strategy)(req, res, function(){
                            res.send(200,req.user);
                        });
                    } catch(e) {
                        e.status = 500;
        ...
        app.all('/api*', function(req,res,next) {
            ...
            req.isAuthenticated = function hubbaIsAuthenticatedDef(){
                if (req.session && req.session.passport && req.session.passport.user && req.session.passport.user.hubbaSession){
                    return false;
                }
                return isAuth;
            };
