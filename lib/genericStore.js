...
function GenericStore(opts){
    ...
    assert.ok(aCnt === requiredAdapters.length,'Invalid adapter types, all of the following and only the following must be defined: ' + requiredAdapters.join());
    this._type = o.type;
    this._db = o.db;
    this._adapters = o.adapters;

    for ( var i in requiredAdapterFunctions ){
        this[requiredAdapterFunctions[i]] = this._adapters[this._type][requiredAdapterFunctions[i]];
    }
};
