...
function uploadFile(resource,req,res){
    ...
    k.forEach(function(f){
        ...
        var reader = fs.createReadStream(files[f].path);

        reader.on('end', function() {
            fs.unlink(files[f].path);
            cnt--;
            if (cnt === 0){
                res.send(200,resource.path.replace('*','') + '/' + files[f].name);
            }
        });
        ...
        reader.pipe(fs.createWriteStream(resource.configuration.path + '/' + files[f].name));
    });
};
