...
function ResourceTypeFactory(opts, mocks){
        ...
        ResourceTypeFunction = function(cfg){
            ...
            this.on('validate',function(){
                if (ResourceType.validate){
                    ResourceType.validate(self.toHTTPJSON());
                }
            });
            this.on('update',this.updateConfiguration);
            this.on('update',function(){
                if (ResourceType.update){
                    ResourceType.update(self.toHTTPJSON());
                }
            });
            ...
            this.on('destroy',function(){
                if (ResourceType.destroy){
                    ResourceType.destroy(self.toHTTPJSON());
                }
            });
            ...
            if (ResourceType.GET){
                this.http.GET = function(req,res){
                    ResourceType.GET(self.toHTTPJSON(),req,res);
                }
            }

            if (ResourceType.POST){
                this.http.POST = function(req,res){
                    ResourceType.POST(self.toHTTPJSON(),req,res);
                }
            }

            if (ResourceType.PUT){
                this.http.PUT = function(req,res){
                    ResourceType.PUT(self.toHTTPJSON(),req,res);
                }
            }

            if (ResourceType.PATCH){
                this.http.PATCH = function(req,res){
                    ResourceType.PATCH(self.toHTTPJSON(),req,res);
                }
            }

            if (ResourceType.DELETE){
                this.http.DELETE = function(req,res){
                    ResourceType.DELETE(self.toHTTPJSON(),req,res);
                }
            }

            if (ResourceType.init){
                ResourceType.init(self.toHTTPJSON());
            }
        };
    ...
    ResourceTypeFunction.prototype.toHTTPJSON = ResourceTypeToHTTPJSON
...
function ResourceTypeToHTTPJSON() {
    var r = this.toJSON();
    r.data = this.data;
    return r;
};
...
function ResourceTypeToJSON(opts){
    ...
    for ( var key in this.configuration ){
        if (this.ResourceTypeFactory.configuration[key].inputType === 'password'){
            j.configuration[key] = '';
        } else {
