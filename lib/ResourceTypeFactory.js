...
var assert = require('assert-plus'),
    util = require('util'),
    rResource = require('./service/service.js').Service,
...
function ResourceTypeFactory(opts, mocks){
    ...
    var thisFactory = this,
        ResourceTypeFunction = function(cfg){
            ...
            this.configuration = {};
            this.configurationKeys = this.ResourceTypeFactory.configurationKeys;
            this.data = {};
            ...
            if (ResourceType.init){
                try {
                    ...
                    console.log('Resource id: '+this.id+ ' threw an error on init: '+e);
                }
            }
        };
...
function ResourceTypeValidateConfiguration(update){
    ...
    if (cfg){
        for ( var key in cfg ){
            ...
            if (typeof this.ResourceTypeFactory.configuration[key] == 'undefined'){
                throw new Error("This resource type does not have a configuration item called " + key +".");
            ...
            //	TODO, check more specifically for types like email, number ranges, colors, etc...
                ...
                throw new Error(key + " must be of type " + Object.prototype.toString.call(this.ResourceTypeFactory.configuration[key].value) + ", instead is of type " + Object.prototype.toString.call(cfg[key]) + ".");
            ...
            if ( this.ResourceTypeFactory.configuration[key].required === true &&
                this.ResourceTypeFactory.configuration[key].inputType !== 'number' &&
                cfg[key].length === 0 ) {
    ...
    for ( var key in this.ResourceTypeFactory.configuration ) {
        if ( this.ResourceTypeFactory.configuration[key].required === true &&
             this.ResourceTypeFactory.configuration[key].inputType !== 'number' &&
