...
function UserAPIController(opts) {
    ...
    var _store = o.userStore,
        _sessionSecret = o.sessionSecret || 'shhhhh', // TODO: a real secret would be good...
        _tokenOptions = {
            expiresInMinutes: o.expiresInMinutes || 30,
            audience: 'hubba-admins'
        };
    ...
    this.login = function(username,password,done) {
        _store.findByUsername(username, function userResponseFunction(err,user) {
            ...
            user.verifyPassword(password, function(err,valid) {
                ...
                var u = user.toJSON();
                u.token = jwt.sign(u, _sessionSecret, _tokenOptions);
    ...
    this.validateToken = function validateTokenDef(token, done) {
        jwt.verify(token, _sessionSecret, _tokenOptions, function(err, decoded){
            if (err) {
                err.status = 401;
            }
            done(err, decoded);
