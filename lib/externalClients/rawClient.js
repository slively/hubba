...
(function() {
                ...
                promise: {
                    then: function (onSuccess,onFail) {
                        return registerThenHandler(function(obj){
                            onSuccess(obj.body || obj,obj.code);
                        }, function(obj){
                            onFail(obj.body || obj,obj.code);
                        });
                    },
                    ...
                    fail: function (onFail) {
                        ...
                        return registerThenHandler(function(obj){
                            onEnsure(obj.body || obj,obj.code);
                        }, function(obj){
                            onEnsure(obj.body || obj,obj.code);
                        });
                    }
    ...
    var socket = io.connect(null, {resource:'sockets'}),
        sessionStore = window.sessionStorage,
        loginPopupPromise,
        access_token,
        updateAccessToken = function updateAccessTokenDef(token){
            sessionStore.hubbaApiAccessToken = access_token = token;
        },
        removeAccessToken = function removeAccessTokenDef(){
            access_token = undefined;
            delete sessionStore.hubbaApiAccessToken;
        };

    try {
        sessionStore.testing = true;
        delete sessionStore.testing;
        access_token = sessionStore.hubbaApiAccessToken;
    } catch(e) {
        sessionStore = {};
    }
    ...
    if (!window.hubba) {
        window.hubba = {
            ...
            login: function loginDef(strategyName, body, options){
                return ajax('post','/api/login/'+strategyName,body,options).success(function loginSuccessCB(user){
                    updateAccessToken(user.access_token);
                }).error(function loginErrorCB(err){
                    removeAccessToken();
                });
            },
            logout: function logoutDef() {
                removeAccessToken();
            },
            loginPopup: function loginPopupDef(strategyName, options){
                var o = options || {},
                    width = o.width || 800,
                    height = o.height || 580;

                window.open('/api/login/'+strategyName, 'Login', 'width='+width+',height='+height);
                loginPopupPromise = ayepromise.defer();
                return loginPopupPromise.promise;
            },
            loginPopupCallback: function loginPopupCallbackDef(user){
                if (loginPopupPromise) {
                    updateAccessToken(user.access_token);
                    loginPopupPromise.resolve(user);
                    loginPopupPromise = undefined;
                }
            },
            loginPopupErrorCallback: function loginPopupErrorCallbackDef(err){
                if (loginPopupPromise) {
                    removeAccessToken();
                    loginPopupPromise.reject(err);
                    loginPopupPromise = undefined;
                }
            }
        };
