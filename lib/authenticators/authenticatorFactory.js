...
AuthenticatorFactory.prototype.bundleAuthenticatorForPassport = function bundleAuthenticatorForPassportDef(opts) {
    ...
    assert.string(passportAuthenticator.name,authErrName +'name');
    assert.notEqual(passportAuthenticator.name,'session','Authenticator name cannot be \'session\'.');
    assert.ok(passportAuthenticator.name.toLowerCase().indexOf('hubba') === -1, 'Authenticator name cannot contain \'hubba\'.');
    assert.optionalString(passportAuthenticator.code,authErrName +'code');
    ...
    Object.keys(o.configuration).forEach(function(key){
        ...
        passportAuthenticator.configuration[key] = o.configuration[key];
    });

    // validate required properties are present
    _.forOwn(this.configuration,function(value,key) {
        if (value.required) {
            assert.ok(passportAuthenticator.configuration[key],authErrName +'configuration key \'' + key + '\' is required');
        }
    });

    return {
        name: passportAuthenticator.name,
        passportArgs: [
            passportAuthenticator.name,
            new this.strategy(
                passportAuthenticator.configuration,
                eval("(function(" + _.pluck(this.args, 'name').join() + ",done){" + passportAuthenticator.code + "});")
            )
        ]
    };
};
