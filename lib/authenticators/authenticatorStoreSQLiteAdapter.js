...
exports.Adapter = {
    connected: function authenticatorConnected(d){
        ...
        self._db.serialize(function(){
            self._db.run("CREATE TABLE IF NOT EXISTS authenticators (id INTEGER PRIMARY KEY AUTOINCREMENT, version INTEGER DEFAULT 1 NOT NULL, name TEXT NOT NULL UNIQUE, strategy TEXT NOT NULL, configuration TEXT, code TEXT);", function (err) {
                if(err) done(err);

                self._db.run("CREATE TABLE IF NOT EXISTS authenticator_versions (authenticator_id INTEGER, version INTEGER NOT NULL, name TEXT NOT NULL, strategy TEXT NOT NULL, configuration TEXT, code TEXT, FOREIGN KEY(authenticator_id) REFERENCES authenticators(id), PRIMARY KEY (authenticator_id, version));", function(err){
                    if(err) done(err);
                    done();
                });
            });
    ...
    add: function authenticatorAdd(authenticator,cb){
        ...
        self._db.run(
            'INSERT into authenticators (name,strategy,configuration,code) VALUES(?,?,?,?);',
            [authenticator.name, authenticator.strategy, authenticator.configuration, authenticator.code],
            function(err) {
                cb(err,this.lastID);
            }
        );
    ...
    replace: function authenticatorReplace(authenticator,cb){
        ...
        self._db.run(
            'INSERT into authenticator_versions (authenticator_id, name, version, strategy, configuration, code) select id, name, version, strategy, configuration, code from authenticators where id = ?;',
            [authenticator.id],
            function(err){
                if (err)
                    cb(err);
                ...
                self._db.run(
                    'UPDATE authenticators set version = version + 1, name = ?, strategy = ?, configuration = ?, code = ? where id = ?;',
                    [authenticator.name, authenticator.strategy, authenticator.configuration, authenticator.code, authenticator.id],
                    function(err){
                        if (this.changes === 0) {
                            cb(new Error('No auth authenticator found with id ' + authenticator.id));
                        } else {
                            cb(err, this.changes);
                        }
                    }
