...
function ServiceRouter (opts,mocks){
...
ServiceRouter.prototype._addRoute = function _addRoute(opts) {
    ...
    verbs.forEach(function(verb){
       ...
       // self.server.routes[verb][self.server.routes[verb].length-1].id = opts.id;
        //self.server.routes[verb][self.server.routes[verb].length-1].seq = opts.seq;
