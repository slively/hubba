...
ServiceRouter.prototype._addRoute = function _addRoute(opts) {
    ...
    verbs.forEach(function(verb){
        self.server[verb]('/api' + opts.path, function(req,res,next){
            ...
            // if there is are _mocks for this request, and there is a mock that matches
            //  then send the mock response
            if (req._mocks) {
                for ( var i = 0; i < req._mocks.length; i++) {
                    if (req._mocks[i].req.path === '/api' + opts.path && req._mocks[i].req.method === verb.toUpperCase()) {
                        req._mocks[i].executed = true;
                        res.send(req._mocks[i].res.statusCode, req._mocks[i].res.body);
                        return;
                    }
                }
            }
