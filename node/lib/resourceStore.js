...
function SQLiteStore(opts){
    ...
    this.add = function add(resource,cb){
        ...
        if (connecting){
            ...
            db.run('INSERT into resources (resource) VALUES (?);',[JSON.stringify(resource)],function(err){
                if (err) throw err;
                cb(err,this.lastID);
            });
        }
    ...
    this.replace = function replace(resource,cb){
        ...
        if (connecting){
            ...
            db.run('UPDATE resources set resource = ? where id = ?;',[JSON.stringify(resource),resource.id],function(err){
                if (err) throw err;
                cb(err,this.changes);
            });
        }
