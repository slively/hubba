...
/*
describe('controller resource NODE API tests', function() {
	...
	it(' should create a new controller resource with the name controller_resource',function(done){
		r = resource.findById(root.id).addChild({
			parentId: root.id,
			name: "controller_resource",
			type: "controller"
		}).toJson();
		assert.equal(r.type,"controller","Rest resource has incorrect type! should be 'controller', is actually " + r.type);
		assert.equal(r.name,"controller_resource","Rest resource has incorrect name! shouldbe be 'controller_resource is actually " + r.name);
		done();
	})
	
	it ('should update the controller resource with a new get code, post code, put code, patch code, and delete code.',function(done){
	...
	// do some invalid configuration tests...

	it (' should delete the controller resource.',function(done){
